...
`layout.content_width` kiest `standard` of `wide`; de brede standaard gebruikt vier Sections-kolommen. `layout.navigation_mode` kiest `native`, `integrated` of `kiosk`. Kiosk levert interne navigatie en `kiosk_mode.hide_header`; het werkelijk verbergen van de Home Assistant-bovenbalk vereist de optionele kiosk-mode frontendresource of een kioskfunctie van de Companion App.
