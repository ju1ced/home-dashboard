...
Schema v1 behoudt compatibiliteit. Per kamer zijn `home_favorite` en `controls_enabled` booleans met migratiestandaard `false`. De optionele `control_entities`-array bevat nul tot zestien unieke licht-, cover-, mediaspeler- of klimaatentiteiten; de arrayvolgorde is de zichtbare chipvolgorde en domeinen mogen meermaals voorkomen. Ontbreekt het veld, dan blijft de bestaande bediening uit `control_light_entity`, `control_cover_entity`, `control_awning_entity`, `control_media_entity` en de bronmappings zichtbaar als migratiepad. Zodra de editor de lijst opslaat, is ook een lege array leidend. Maximaal vier `home_favorite`-kamers; de kamervolgorde bepaalt hun volgorde op Home. De twee legacy `quick_actions` blijven afzonderlijk opgeslagen en zijn geen capabilityknoppen.

`layout.content_width` kiest `standard` of `wide`; de brede standaard gebruikt vier Sections-kolommen. `layout.navigation_mode` kiest `native`, `integrated` of `kiosk`. Geïntegreerd en kiosk gebruiken dezelfde navigatiecard als eerste sectie op vervolgviews en componeren die binnen de responsieve Home-header op Home. De lokale navigatiecorrectie geeft de kioskmodus door aan die card in plaats van een extern `kiosk_mode.hide_header`-veld te genereren. De begrensde shelladapter verbergt de HA-bovenbalk, ruimt bij vertrek op en respecteert `?disable_km` en native edit mode. Het schema verandert niet. Zie [navigatie en herstel](../configuration/gui-overview.md#navigatie).
