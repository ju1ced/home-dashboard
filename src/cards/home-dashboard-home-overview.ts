...
export function getHomeStructureSignature(hass: HomeAssistantLike | undefined, config: HomeOverviewConfig): string {
  ...
  return [...attention, "rooms", ...rooms].join("|");
}
...
function specialistNavigationLink(label: string, subtitle: string, iconName: string, path: string): HTMLAnchorElement {
  const link = navigationLink(label, iconName, path);
  link.classList.add("specialist-link");
  ...
  labelNode?.replaceWith(copy);
  const arrow = document.createElement("ha-icon") as HTMLElement & { icon?: string };
  arrow.className = "nav-arrow";
  arrow.icon = "mdi:chevron-right";
  link.append(arrow);
  return link;
}
...
export class HomeDashboardHomeOverview extends HTMLElementBase {
  ...
  private navigationCard?: HTMLElement & { setConfig: (config: Record<string, unknown>) => void; hass?: HomeAssistantLike };
  ...
  private updateLiveState(): void {
    ...
    if (hass && this.navigationCard && this.config.navigation_mode !== "native") this.navigationCard.hass = hass;
  ...
  private async render(): Promise<void> {
    ...
    top.className = config.navigation_mode && config.navigation_mode !== "native" ? "top with-navigation" : "top";
    if (top.classList.contains("with-navigation")) {
      const navigation = this.navigationCard ??= document.createElement("home-dashboard-navigation") as HTMLElement & { setConfig: (config: Record<string, unknown>) => void; hass?: HomeAssistantLike };
      navigation.setConfig({ type: "custom:home-dashboard-navigation", active: "home", navigation_mode: config.navigation_mode ?? "integrated", ...(config.palette ? { palette: config.palette } : {}), theme_mode: config.theme_mode ?? "system" });
      if (hass) navigation.hass = hass;
    ...
    const buildSpecialistNavigation = (compact = false): HTMLElement | undefined => {
      const specialistNames: Record<keyof SpecialistsConfig, [string, string, string, string]> = {
        kia: ["Auto", "Voertuig", "mdi:car-electric", "specialist-kia"], printer: ["Printer", "Werkplaats", "mdi:printer-3d-nozzle", "specialist-printer"], robot: ["Robot", "Schoonmaak", "mdi:robot-vacuum", "more"], garden: ["Tuin", "Buiten", "mdi:flower", "more"], pool: ["Zwembad", "Waterkwaliteit", "mdi:pool", "specialist-pool"]
      };
      ...
      wrapper.className = "today-navigation";
      const label = document.createElement("span");
      label.className = "nav-label";
      label.textContent = "Snel naar";
      wrapper.append(label, navGrid);
      return wrapper;
    };
    ...
    if (config.today?.enabled) {
      ...
      const compactNavigation = buildSpecialistNavigation(true);
      if (compactNavigation) side.append(compactNavigation);
    ...
    if (!config.today?.enabled) {
      ...
      if (navGrid) {
        ...
        navigation.append(sectionHeader("Snel naar", "Volledige specialistische dashboards"), navGrid);
