...
export class HomeDashboardHomeOverview extends HTMLElementBase {
  ...
  private async render(): Promise<void> {
    ...
    style.textContent = `
      ...
      @media(max-width:1180px){.top{flex-wrap:wrap}.today-grid.with-security{grid-template-columns:repeat(2,minmax(0,1fr))}.today-main,.security-panel{grid-column:1/-1}.security-panel{grid-row:auto}}
    ...
    top.className = config.navigation_mode && config.navigation_mode !== "native" ? "top context" : "top";
    ...
    if (config.navigation_mode && config.navigation_mode !== "native") {
      // Navigation belongs to the shared first section on every view.
      // Keep Home context below it without a second coloured navigation bar.
      top.style.background = "transparent";
      top.style.color = "var(--hd-text)";
