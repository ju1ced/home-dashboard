...
export class HomeDashboardRoomDetail extends RoomCardBase<RoomDetailConfig> {
  ...
  private render(): void {
    ...
    const style = document.createElement("style");
    style.textContent = `
      :host{display:block;min-width:0}.detail{display:grid;gap:22px;width:100%;margin:0 auto}.hero{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:24px;border-radius:22px;background:var(--primary-color,#245c4d);color:var(--text-primary-color,#fff)}.hero-copy{display:grid;gap:4px}.eyebrow{font-size:.72rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;opacity:.75}.hero h1{margin:0;font-size:1.85rem}.hero p{margin:0;opacity:.75}.hero-actions{display:grid;justify-items:end;gap:10px}.hero-pills{display:flex;justify-content:flex-end;gap:7px;flex-wrap:wrap}.hero-pill{padding:7px 10px;border:1px solid color-mix(in srgb,currentColor 28%,transparent);border-radius:999px;font-size:.78rem;font-weight:650}.home-link{display:inline-flex;align-items:center;gap:7px;min-height:44px;padding:8px 12px;border:1px solid color-mix(in srgb,currentColor 38%,transparent);border-radius:12px;color:inherit;text-decoration:none;font-size:.82rem;font-weight:700}.home-link:hover{background:color-mix(in srgb,currentColor 12%,transparent)}.home-link:focus-visible{outline:2px solid currentColor;outline-offset:2px}.home-link ha-icon{width:20px;height:20px}
    ...
    homeLink.href = "home";
    homeLink.append(icon("mdi:home-outline"), document.createTextNode("Home"));
    homeLink.setAttribute("aria-label", "Ga naar Home");
    heroActions.append(heroPills, homeLink);
