...
export class HomeDashboardNavigation extends HTMLElementBase {
  ...
  private render(): void {
    ...
    style.textContent = `:host{display:block;min-width:0;--hd-hero:var(--primary-color,#087fb9)}*{box-sizing:border-box}nav{display:flex;align-items:center;gap:7px;min-height:64px;padding:10px 14px;border-radius:18px;background:var(--hd-hero);box-shadow:0 1px 2px rgb(20 35 28/.06),0 7px 24px rgb(20 35 28/.035);overflow-x:auto;scrollbar-width:none}nav::-webkit-scrollbar{display:none}a{display:flex;flex:0 0 auto;align-items:center;gap:7px;min-height:42px;padding:8px 12px;border:1px solid transparent;border-radius:12px;color:#fff;text-decoration:none;font-size:.84rem;font-weight:700}a:hover,a:focus-visible{background:rgb(255 255 255/.15);border-color:rgb(255 255 255/.35);outline:0}a[aria-current=page]{background:#fff;color:var(--hd-hero)}ha-icon{width:20px;height:20px}@media(max-width:560px){nav{border-radius:15px;padding:8px}a{min-width:44px;justify-content:center;padding:8px}a span{display:none}}`;
